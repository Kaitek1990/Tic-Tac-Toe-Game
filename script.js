...
function drawWinningLine(combination) {
  ...
  // Start- und Endzellen der Kombination
  const startCell = document.querySelectorAll('td')[combination[0]];
  const endCell = document.querySelectorAll('td')[combination[2]];

  // Rechtecke der Start- und Endzellen relativ zum 'content'-Element
  const startRect = startCell.getBoundingClientRect();
  const endRect = endCell.getBoundingClientRect();

  // Rechteck des 'content'-Elements relativ zum Dokument
  ...
  // Position der Linie relativ zum 'content'-Element
  ...
  // Linie zum 'content'-Element hinzufügen
